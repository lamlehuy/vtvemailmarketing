			...
			<div class="loginDiv widgetHeight">
				...
				<div id="loginFormDiv">
					<form class="form-horizontal" method="POST" action="index.php">
						...
						<div class="group">
							<input id="username" type="text" name="username" placeholder="Tên truy cập" value="admin">
							<span class="bar"></span>
							<label>Tên truy cập</label>
						</div>
						<div class="group">
							<input id="password" type="password" name="password" placeholder="Mật khẩu" value="admin">
							<span class="bar"></span>
				...
				<div id="forgotPasswordDiv" class="hide">
					<form class="form-horizontal" action="forgotPassword.php" method="POST">
						<div class="group">
							<input id="username" type="text" name="username" placeholder="Tên truy cập" >
							<span class="bar"></span>
							<label>Tên truy cập</label>
						</div>
						<div class="group">
						...
						<div class="group">
							<button type="submit" class="button buttonBlue forgot-submit-btn">Gửi</button><br>
							<span>Hãy nhập tên truy cập và email của bạn<a class="forgotPasswordLink pull-right" style="color: #15c;">Quay lại</a></span>
						</div>
					</form>
				</div>
			</div>
		...
		<script>
			jQuery(document).ready(function () {
				...
				loginFormDiv.find('button').on('click', function () {
					...
					var errorMessage = '';
					if (username === '') {
						errorMessage = 'Chưa nhập tên truy cập';
						result = false;
					} else if (password === '') {
				...
				forgotPasswordDiv.find('button').on('click', function () {
					...
					if (username === '') {
						...
						errorMessage = 'Địa chỉ email có chứa các ký tự không hợp lệ.';
						result = false;
					}
					if (errorMessage) {
						validationMessage.removeClass('hide').text(errorMessage);
