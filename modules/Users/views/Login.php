...
class Users_Login_View extends Vtiger_View_Controller {
	...
	function process (Vtiger_Request $request) {
		...
		if ($error) {
			switch ($error) {
				case 'login'		:	$message = 'Tên truy cập hoặc mật khẩu không đúng';						break;
				case 'fpError'		:	$message = 'Tên truy cập hoặc email không đúng';			break;
				case 'statusError'	:	$message = 'Chưa thiết lập máy chủ gửi mail';	break;
			}
		} else if ($mailStatus) {
			$message = 'Hệ thống đã gửi thông tin qua email cho bạn, hãy kiểm tra email.';
		}
