...
class Vtiger_Util_Helper {
     ...
     * @return <String>
     */
    public static function formatDateTimeIntoDayStringVN($dateTime, $skipConversion = FALSE) {
        $currentUser = Users_Record_Model::getCurrentUserModel();
        $dateTimeInUserFormat = $dateTime;
        if (!$skipConversion) {
            $dateTimeInUserFormat = Vtiger_Datetime_UIType::getDisplayDateTimeValue($dateTime);
        ...
        $formatedDate .= vtranslate('LBL_'.date('D', $date)). ', ' .date('d', $date).'/'.date('m', $date). '/' .date('Y', $date);
        //Adding time details
